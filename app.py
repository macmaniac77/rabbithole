...
from database import SessionLocal, engine, create_db_and_tables, DBUserContext, DBValuePoint, DBAuthUser
from models import UserContext as PydanticUserContext
from git_ops import prepare_commit_message, commit_and_push, ensure_git_repo as initialize_git_repo_at_startup
from llm_strategies import get_prompt_for_bigger, get_prompts_for_deeper

app = Flask(__name__)
bcrypt = Bcrypt(app)

# Initialize database and Git repository
...
if not GOOGLE_API_KEY:
    ...
    try:
        ...
        print(f"Warning: Gemini API configuration failed: {e}. App will start, but Gemini calls may fail.")
...
def jwt_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        if not auth_header or not auth_header.startswith("Bearer "):
            return jsonify({"message": "Auth header missing/invalid"}), 401
...
def create_vp(db: Session, user_id: str, title: str, vp_type: Literal["payment", "contract", "task", "earn", "settle"], 
              interface: str, price_usd: Optional[float] = None, price_sat: Optional[int] = None, 
              expires: Optional[datetime] = None, creditable: bool = True, btc_commit: bool = False, 
              next_vps: List[str] = []) -> DBValuePoint:
    vp_id = str(uuid.uuid4())
    user_context = get_or_create_user_context(user_id, db)
    new_vp = DBValuePoint(id=vp_id, title=title, vp_type=vp_type, price_usd=price_usd, price_sat=price_sat, 
                          expires=expires, interface=interface, creditable=creditable, btc_commit=btc_commit, next=next_vps)
    db.add(new_vp)
    if new_vp not in user_context.active_vps_rels:
        user_context.active_vps_rels.append(new_vp)
    return new_vp
...
def spawn_child_vps(db: Session, parent_vp: DBValuePoint, user_id: str):
    ...
    if parent_vp.vp_type == "contract" and parent_vp.title == "Basic Content Creation Contract":
        ...
        newly_spawned_vps.append(create_vp(db, user_id, "Fulfill Order", "task", "ui_interfaces/task_fulfill_order.md"))
...
def register():
    ...
    db = get_db_session()
    try:
        if db.query(DBAuthUser).filter_by(username=username).first():
            return jsonify({"message": "User exists"}), 400
        new_user = DBAuthUser(username=username)
        new_user.set_password(password)
        db.add(new_user)
        get_or_create_user_context(username, db)
        db.commit()
...
def view_document(doc_path_str: str):
    ...
    if not full_path.is_file():
        abort(404)
    return jsonify({
        "html_content": markdown2.markdown(read_document(full_path)),
        "markdown_content": read_document(full_path)
    })
...
def edit_document():
    ...
    if not doc_path or content is None:
        return jsonify({"message": "doc_path and content required"}), 400
    write_document(BASE_DIR / doc_path, content)
    db = get_db_session()
    try:
        user_ctx = get_or_create_user_context(user_id, db)
        user_ctx.last_input = f"Edited: {doc_path}"
        db.commit()
        commit_and_push(user_id, serialize_user_context_for_git(user_ctx), 
                       prepare_commit_message(user_id, "edited by user", doc_path=doc_path), 
                       source_markdown_relative_paths_to_copy=[doc_path])
...
def generate_document_route():
    ...
    data = request.json
    doc_path = data.get('doc_path')
    operation = data.get('operation', 'alter')
    context_text = data.get('context_text')
    if not context_text:
        return jsonify({"message": "context_text required"}), 400
    
    db = get_db_session()
    try:
        user_ctx = get_or_create_user_context(user_id, db)
        user_ctx.last_input = context_text
        db.commit()
    ...
    if operation == 'alter' and current_doc_full_path and current_doc_full_path.is_file():
        ...
        operation = 'generate'
        prompt_for_llm = f"{user_prompt_info}Generate new document from context:\n\n{context_text}"
    ...
    commit_and_push(user_id, user_context_dict_for_git, 
                   prepare_commit_message(user_id, f"{operation} LLM", doc_path=final_doc_path_rel), 
                   source_markdown_relative_paths_to_copy=[final_doc_path_rel])
...
def webhook_stripe():
    ...
    if not webhook_secret:
        return jsonify({"error": "Stripe webhook secret not set"}), 500
    ...
    try:
        if event['type'] == 'checkout.session.completed':
            ...
            is_public_contribution = metadata.get('is_public_contribution', 'false').lower() == 'true'
            ...
            if raw_path_history:
                try:
                    ...
                    if isinstance(loaded_history, list):
                        ...
                        print(f"Warning: document_path_history from Stripe metadata was not a list: {raw_path_history}")
                except json.JSONDecodeError:
                    print(f"Warning: Failed to parse document_path_history JSON from Stripe: {raw_path_history}")
            ...
            if auth_user:
                ...
                payment_vp_instance = create_vp(db, client_user_id, payment_vp_title, "payment", 
                                              "ui_interfaces/payment_receipt.md", price_usd=float(session.get('amount_total',0)/100.0))
                ...
                if document_path_history_list:
                    ...
                    if prev_doc_full_path.is_file():
                        try:
                            ...
                            print(f"Error reading hist doc {prev_doc_rel_path}: {e_read}")
                ...
                if not target_doc_full_path.is_file():
                    ...
                    try:
                        ...
                        if action_type == "bigger":
                            ...
                            response = gemini_model.generate_content(prompt_user_info + prompt)
                            llm_response = response.text.strip()
                            if llm_response:
                                write_document(target_doc_full_path, llm_response)
                                files_for_overall_git_commit.append(doc_id_rel_str)
                                llm_action_taken = True
                                llm_action_description_for_commit = f"LLM 'bigger' on {Path(doc_id_rel_str).name}"
                            ...
                            response = gemini_model.generate_content(prompt_user_info + prompt, generation_config=generation_config)
                            ...
                            try:
                                ...
                                if doc_id_rel_str not in files_for_overall_git_commit:
                                    files_for_overall_git_commit.append(doc_id_rel_str)
                                llm_action_taken = True
                                llm_action_description_for_commit = f"LLM 'deeper' on {Path(doc_id_rel_str).name}, new: {new_filename}"
            ...
            if user_context_db_instance:
                ...
                user_context_data_for_git = {
                    ...
                    "details": {
                        ...
                        "doc_id": doc_id_rel_str,
                        "history": document_path_history_list
                    },
            ...
            final_commit_msg = prepare_commit_message(user_id=actor_id_for_git, vp_id=payment_vp_instance.id if payment_vp_instance else None, 
...
def webhook_lightning():
    ...
    try:
        ...
        payment_vp = create_vp(db=db, user_id=user_id, 
                              title=f"Lightning Payment: {amount_sat} sats (Hash: {payment_hash[:8] if payment_hash else 'N/A'}...)", 
                              vp_type="payment", interface="ui_interfaces/payment_receipt.md", price_sat=amount_sat)
        ...
        db.rollback()
        print(f"Error processing Lightning webhook: {e}")
        import traceback
        traceback.print_exc()
...
def vp_complete():
    ...
    if not vp_id:
        return jsonify({"message": "vp_id is required"}), 400
    db = get_db_session()
    try:
        user_context_db = get_or_create_user_context(user_id, db)
